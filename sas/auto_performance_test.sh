...
function disk_data_comprehensive_test()
{
    ...
    do
        ...
        do
            ...
            if [ x"$temp_info" = x"" ]
            then
                fail_test "Mount "$dev_part" disk failure,test case execution failed."
            ...
            then
                umount $dev_part
                fail_test "dd tools read data error,test case execution failed."
            ...
            if [ x"$temp_info" != x"" ]
            then
                fail_test "Failed to uninstall the "$dev_part" disk,test case execution failed."
            ...
            do
                ...
                then
                    fail_test "when useing fio to read and write $dev_part partition failed,test case execution failed."
...
function runing_fio_link_reset()
{
    ...
    if [ x"$runing_fio_link_reset_file_name" = x"" ]
    then
    ...
    ./fio -filename=$runing_fio_link_reset_disk -direct=1 -iodepth 1 -thread -rw=randwrite \
        ...
        -group_reporting -stonewall -name=mytest 1>$ERROR_INFO 2>&1 &
	...
	end_time=`date +%s`
        if [ `expr $end_time-$begin_time` -ge `2*runing_fio_link_reset_time` ]
        then
...
function runing_fio_hard_reset()
{
    ...
    ./fio -filename=$runing_fio_hard_reset_disk -direct=1 -iodepth 1 -thread -rw=randwrite -ioengine=psync -bs=512B \
        -numjobs=64 -runtime=$runing_fio_hard_reset_time -group_reporting -stonewall -name=mytest 1>$ERROR_INFO 2>&1 &
	...
	end_time=`date +%s`
        if [ `expr $end_time-$begin_time` -ge `2*runing_fio_hard_reset_time` ]
        then
            pid=`ps -ef | grep fio | grep -v grep | grep -v vfio-irqfd | awk -F ' ' '{print $2}'`
            kill -9 $pid
    ...
    if [ x"$info" != x"" ]
    then
        fail_test "FIO tools to run the exception,The system runs normally,test case execution failed."
...
function file_transfer_stability_test()
{
    ...
    then
        fail_test "File transfer stability test,IO read and write exception,test case execution failed."
